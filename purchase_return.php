...
<?php
            ...
            <div class="row clearfix">
              ...
              <strong>Sorry !</strong> Item with same Barcode and Serial already Sold.
                   ...
                   <form action="operations/purchase_return.php" method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="body">
                            <div class="row clearfix"> 
                                      ...
                                      ?>
                                        <select class="form-control po"  name="po" onchange="get_detail();" >
                                          <option selected="" value="">-- Chose One --</option>
                                           <?php
                                            $sql="SELECT * FROM tbl_purchase where bill_status='Completed' and payment_status ='Completed' and created_by='$userid'"; 
                                            foreach ($conn->query($sql) as $row){
                  ...
                  <thead style="background-color: orange;">
                    <tr>
                      ...
                      <th style="width:20%" hidden>Product Serial / IEMI</th>
                      <th style="width:20%">Product Barcode</th>
                      <th style="width:10%" hidden>Rtn Qty</th>
                      <th style="width:5%">Qty</th>
                      <th style="width:10%">Rate</th>
                      <th style="width:10%">Amount</th>
                      <th style="width:20%" hidden>Returned Amount</th>
                ...
                <div class="col-md-6 col-xs-12 pull pull-right">
                 ...
                 <div class="form-group" id="cash_in_hand">
                    ...
                    <div class="col-sm-7">
                      <input type="text" class="form-control calculate" id="cash_hand" name="cash_in_hand" placeholder="Cash in hand" onkeyup="subAmount()" autocomplete="off" value="<?php echo round($cash_in_hand+$opening_bal); ?>">
                    </div>
                  </div>
...
var po=$('.po').val();
  ...
  $.ajax({
                url: "operations/purchase_return_items.php",
                type: "POST",
                data: {
                    po : po
                },
...
echo "<option value=$row[item_id]>$row[item_name] "." $row[item_model]</option>"; 
 ...
 ?>'
  ...
  let amountReceivedEdited = false; // Declare the flag

// Add input event listener to detect manual edits
$("#amount_received").on('input', function() {
    amountReceivedEdited = true; // Flag that user is typing
});
 ...
 var discount = $("#discount").val();
    ...
    for(x = 0; x < tableProductLength; x++) {
      ...
      // Only auto-fill amount_received if it hasn't been edited by the user
    if (!amountReceivedEdited) {
        $("#amount_received").val(totalRtnamount); // Autofill if not edited
    }
    } // /for
  ...
  // Function to handle the removal of rows (and resetting the flag)
function removeRow(tr_id) {
    $("#product_info_table tbody tr#row_" + tr_id).remove();
    subAmount();
    amountReceivedEdited = false; // Reset after row removal
}
