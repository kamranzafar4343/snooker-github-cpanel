            ...
            <div class="row clearfix" >
                <div class="col-lg-12 col-md-12">
                    <div class="card invoice1">                        
                        ...
                        $nsql=mysqli_query($conn, "SELECT created_date FROM tbl_purchase_req_detail ");
                        $ndata=mysqli_fetch_assoc($nsql);  // echo $data['c_phone '];exit();
                        $vsql=mysqli_query($conn, "SELECT location FROM tbl_purchase_req where purchase_req_id =$purchase_req_id ");
                        $vdata=mysqli_fetch_assoc($vsql);
                        $v_id = $vdata['location'];
                      
                        $namesql=mysqli_query($conn, "SELECT user_name FROM users where user_id=$v_id");
                        ...
                        <div class="row">
                            ...
                            <span > <b>LOCATION  : </b> <?php echo $username;?></span>
                     </div> 
                            <div class="clearfix text-right col-md-6" >

                            <span > <b> DATE : </b> <?php $created_date=$ndata['created_date']; echo  $newDate = date("d-m-Y H:i a", strtotime($created_date));?>
                       </span>
                     </div> </div> 
                            <hr>  
                                              <div class="row clearfix">
                                                ...
                                                <table id="example" class="display" style="width:100%">
                                                    <thead class="thead-dark">
                                                        <tr>
                                                            ...
                                                            <th>Stock Status</th>
                                                            <th>Stock Recv Status</th>
                                                            <th>Cash Rate</th>
                                                            <th class="hidden-sm-down">Installment Rate</th>
...
<?php
   ...
   $bsql=mysqli_query($conn,"SELECT tbl_purchase_req_detail.*,tbl_items.* From tbl_purchase_req_detail inner join tbl_items on tbl_purchase_req_detail.product=tbl_items.item_id where tbl_purchase_req_detail.purchase_req_id=$purchase_req_id");
...
while($value = mysqli_fetch_assoc($bsql))   
                                {   
                                  ...
                                  // print_r($value);exit();
                                    ...
                                    $cash_rate=$value['cash_rate'];
                                    $inst_rate=$value['inst_rate'];
                                    $qty=$value['qty'];
 ...
 $sql3=mysqli_query($conn, "SELECT stock_receive_status , stock_status FROM tbl_purchase_req where purchase_req_id=$purchase_req_id");
                        $data1=mysqli_fetch_assoc($sql3);
                        $stock_status = $data1['stock_status'];
                        $stock_receive_status = $data1['stock_receive_status'];
...
$count++;
  ?>
                            ...
                            <td><?php echo $stock_receive_status;?></td>
                            
                           
                            <td><?php echo $cash_rate;?></td>
                            <td><?php echo $inst_rate;?></td>
                                                        </tr>
      ...
      buttons: [
        {
          extend: 'pdfHtml5',
          text: 'Alkareem',
           title: 'Alkareem (Pur Req Detail Report) for <?php echo $username;?>',
