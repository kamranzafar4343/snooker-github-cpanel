...
include "../includes/session.php";
	...
	$sql=mysqli_query($conn, "SELECT branch_id, created_by  FROM users where user_id='$location'");
...
if($edit_id!='')
{
	...
	$sql=mysqli_query($conn, "DELETE FROM tbl_trans_detail WHERE invoice_no = '$invoice_no'");
     ...
     for ($a = 0; $a < count($_POST["qty"]); $a++)
        {
            $sql = "INSERT INTO tbl_purchase_detail (purchase_id, invoice_no, product, qty, cash_rate, inst_rate, created_date, created_by) VALUES ('$edit_id', '$invoice_no','" . $_POST["product"][$a] . "', '" . $_POST["qty"][$a] . "',  '" . $_POST["rate"][$a] . "',  '" . $_POST["amount"][$a] . "', '$created_date', '$created_by')";
