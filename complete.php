...
if (isset($_POST['sale_ids'])) {
    ...
    foreach ($sale_ids as $sale_id) {
        ...
        // Update sale status
        $sql = mysqli_query($conn, "Update tbl_sale set status='1', sale_type = 'Cash',  sale_status='Completed',net_amount = '$gross_amount', gross_amount= '$gross_amount', amount_recieved='$gross_amount', completed_at = '$completed_at', created_by = '$userid', payment_type = 'cash' where sale_id='$sale_id'");
       ...
       bill_status, narration, created_date, created_by, parent_id, numbering) 
               ...
               VALUES ('$tranid', '$ref_id', '$cash_in_hand',  '$amount_recieved', '0.00','cash', 'Completed',
               '$narration', '$completed_at', '$userid', '$parent_id', 'debit cash in hand - cm1')");

            // credit customer
            $sql2 = mysqli_query($conn, "INSERT INTO tbl_trans_detail (trans_id, invoice_no, acode, d_amount, c_amount,payment_type, 
        bill_status, narration, created_date, created_by, parent_id, numbering) 

                VALUES ('$tranid', '$ref_id', '$credit_customer', '0.00', '$amount_recieved','cash','Completed', 
                '$narration', '$completed_at', '$userid', '$parent_id' , 'credit customer - cm2')");

            //then update there invoices also
            $sql3 = mysqli_query($conn, "Update tbl_trans_detail SET narration = '$narration', payment_type = 'cash' WHERE invoice_no = '$ref_id'");
        } else {
    ...
    if ($count_data < 8) {
       ...
       bill_status, narration, created_date, created_by, parent_id, numbering) 
               ...
               VALUES ('$tranid', '$ref_id', '$cash_in_hand',  '$amount_recieved', '0.00','cash', 'Completed',
               '$narration', '$completed_at', '$userid', '$parent_id' , 'debit cash in hand - cs1')");
        ...
        bill_status, narration, created_date, created_by, parent_id, numbering) 
                ...
                VALUES ('$tranid', '$ref_id', '$credit_customer', '0.00','$amount_recieved','cash' ,'Completed', 
                '$narration', '$completed_at', '$userid', '$parent_id' , 'credit customer - cs2')");
