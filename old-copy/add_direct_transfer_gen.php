...
?>
    ...
    <div id="main-content">
        <div class="container-fluid">
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a> Create New PR Transfer</h2>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.php"><i class="icon-home"></i></a></li>
                            <li class="breadcrumb-item">Purchase</li>
                            <li class="breadcrumb-item active">Add PR Transfer</li>
                        </ul>
                    </div>            
            ...
            <div class="alert alert-danger" id="stock_alert" style="display: none;">
...
<?php
              ...
              if(isset($_GET['edit_id']))
              {
                ...
                $data=mysqli_fetch_assoc($sql);
                       ...
                       $item_recieved = $data['item_transfer'];
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                   <form action="operations/add_direct_trasfer_iemi.php" method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="body">
                            <div class="row clearfix"> 
                                      ...
                                      ?>
                                        <select class="form-control po"  name="po" onchange="get_detail();" >
                                          <option selected="" value="">-- Chose One --</option>
                                           <?php
                                            $sql="SELECT * FROM tbl_purchase_req where stock_status='Pending' and iemi='0' and transfer_type='1'"; 
                                            foreach ($conn->query($sql) as $row){
                                              $location=$row[parent_id];
                                            ...
                                            if($row['purchase_req_id']==$purchase_req_id)
                                            {
                                            echo "<option value=$row[purchase_req_id] selected>$row[purchase_req_id]</option>"; 
                                            }
                                            else
                                            {
                                            echo "<option value=$row[purchase_req_id]>$row[purchase_req_id] - $to_location_name - $row[po_remarks] </option>"; 
                                            }
                                            }
                  ...
                  <thead style="background-color: orange;">
                    <tr>
                      ...
                      <th style="width:10%" hidden>Item Serial</th>
                      <th style="width:20%">Item Barcode</th>
                      <th style="width:10%">Pur Rate</th>
                      <th style="width:5%" hidden="">Stock</th>
                     
                      <th style="width:5%" hidden="">Transfer</th>
                      <th style="width:5%">Qty</th>
                      <th style="width:10%">Cash Rate</th>
                      <th style="width:10%">Installment Rate</th>
                ...
                <div class="col-md-6 col-xs-12 pull pull-right" hidden>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-7">
                      <input type="text" class="form-control" id="gross_amount" name="gross_amount" readonly="" autocomplete="off" value="0">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-7">
                      <input type="text" class="form-control" id="net_amount" name="net_amount" autocomplete="off" readonly="" value="0">
                    </div>
                  </div>
...
$.ajax({
                                  method: "POST",
                                  url: "operations/get_pr_detail.php",
                                  data: {po:po},
                                  dataType: 'json',                 
                                })
...
var edit_id=$('#edit_id').val();
  ...
  $.ajax({
                url: "operations/add_req_item_gen.php",
                type: "POST",
                data: { 
                    po : po,
                    edit_id : edit_id
                ...
                success: function(dataResult){
                    ...
                    if(dataResult=="empty"){
                                          $("#stock-alert").hide();
                                          $("#stock-alert").fadeTo(4000, 500).slideUp(500, function() {
   ...
   function get_remaining_items()
 {
    ...
    for(x = 0; x < tableProductLength; x++) {
      ...
      var qty=$("#qty1_"+count).val();
      var cash_rate=$("#cash_rate_"+count).val();
      var inst_rate=$("#inst_rate_"+count).val();
      if(cash_rate=='' || inst_rate=='')
      {
        alert('Please Add Cash and Installment Rate first.')
        $(".submit").hide(); 
      }
     
}

  var dataString = 'itemid='+ itemid + '&qty='+ qty + '&po='+ po;

 
    $.ajax({
type: "POST",
url: "operations/get_remaining_gen.php",
data: dataString,
...
{
    $("#stock_alert").show();
var stoc=$("#stock_alert").html();
 $("#stock_alert").html(responce +"<br>"+ stoc);
}
else
{
    $("#stock_alert").hide();
...
if(net_amount=='0')
{
  alert('Opps! Please atleast transfer one item..!');
    return false;
}
 ...
 ?>'
  ...
  function getTotal(row = null) {
    ...
    if(row) {
      ...
      //  if(Number(stock_qty) < Number(qty_rec))
      // {
      //   alert('Sorry !! Please Enter Qty Less to Stock Qty');
      //   $("#qty_rec_"+row).val(stock_qty);
