...
if (isset($_GET['user_id'])) {
	...
	$sql1 = mysqli_query($conn,"SELECT * from tbl_trans Where parent_id='$del_id' OR  created_by='$del_id'");
	$sql2 = mysqli_query($conn,"SELECT * from tbl_customer Where created_by='$del_id'");
	$sql3 = mysqli_query($conn,"SELECT * from tbl_vendors Where created_by='$del_id'");
		if (mysqli_num_rows($sql1)==0 && mysqli_num_rows($sql2)==0 && mysqli_num_rows($sql3)==0) {

						$sql=mysqli_query($conn, "DELETE FROM users WHERE user_id=$del_id");
						$sql=mysqli_query($conn, "DELETE FROM tbl_permission WHERE user_id=$del_id");
						if($sql){
...
if (isset($_GET['table_id'])) {
	...
	// check if there are entries in tbl_sale
	$sql=mysqli_query($conn, "SELECT table_id FROM tbl_sale WHERE table_id = $table_id");
	if(mysqli_num_rows($sql)>0){
		header('Location: ../table_list.php?delete=unsuccessful');
		exit();
	}
	
		$del_sql=mysqli_query($conn, "DELETE FROM tbl_tables WHERE table_id = $table_id");
		if($del_sql){
			header('Location: ../table_list.php?delete=successful');
		}
		else{
			header('Location: ../table_list.php?delete=unsuccessful');
		}
...
if (isset($_GET['client_type'])) {
	$client_type=mysqli_real_escape_string($conn, $_GET['client_type']);
	$sql = mysqli_query($conn,"SELECT * FROM tbl_customer where customer_type='$client_type'");
	
	if (mysqli_num_rows($sql)==0) {
		$sql=mysqli_query($conn, "DELETE FROM tbl_client_type WHERE type_id=$client_type");
	if($sql){
		header('Location: ../client_type.php?delete=successful');
	}
	}
	else
	{
           ...
           while($pdata = mysqli_fetch_assoc($sql))   
                { 
				   ...
				   $sql2 = mysqli_query($conn, "SELECT * FROM tbl_customer where customer_type='$client_type'");
                	...
                	else
                	{
                		...
                		header('Location: ../client_type.php?delete=unsuccessful');
                	}
	}
