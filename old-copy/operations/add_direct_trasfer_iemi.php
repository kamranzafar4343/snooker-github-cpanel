...
if($edit_id!='')
{
		  ...
		  for ($a = 0; $a < count($_POST["qty"]); $a++)
        {
           ...
           $sql = mysqli_query($conn,"INSERT INTO tbl_purchase_req_detail (purchase_req_id, invoice_no, product, item_serial,pur_item_id, barcode, qty_rec, qty, cash_rate, inst_rate, created_date, created_by, parent_id, trans_id, trans_parent_id,iemi) VALUES ('$tranid', '$invoice_no','" . $_POST["product"][$a] . "', '" . $_POST["item_serial"][$a] . "', '" . $_POST["pur_item_id"][$a] . "', '" . $_POST["barcode"][$a] . "', '" . $_POST["qty_rec"][$a] . "', '" . $_POST["qty"][$a] . "',  '" . $_POST["cash_rate"][$a] . "',  '" . $_POST["inst_rate"][$a] . "', '$created_date', '" . $_POST["created_by"][$a] . "','" . $_POST["parent_id"][$a] . "', '$created_by', '$parent_id', '$iemi')");
            ...
            $sql=mysqli_query($conn, "UPDATE tbl_purchase_detail SET transfer='1' where item_serial='" . $_POST["item_serial"][$a] . "'  and pur_item_id='" . $_POST["pur_item_id"][$a] . "'  and product='" . $_POST["product"][$a] . "'");
        }
...
//             mysqli_query($conn, $sql);
	...
	if($sql){
		header('Location: ../direct_transfer_branch.php?stock=done');
	}
