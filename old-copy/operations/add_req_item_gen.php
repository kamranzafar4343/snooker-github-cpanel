...
$query = "SELECT * FROM tbl_purchase_req_detail where purchase_req_id=$po and qty_rec!='' and pur_item_id='0'";
...
error_reporting(0);
     ...
     while($row = mysqli_fetch_array($result) ){
                        ...
                        while($data5 = mysqli_fetch_array($result5) ){
                         ...
                         $transfer_amount=round($transfer_perc*$pur_rate/100);
                         $transfer_rate=round($transfer_amount+$pur_rate);
...
$return_query="SELECT * FROM tbl_purchase_detail INNER JOIN tbl_items ON tbl_purchase_detail.product = tbl_items.item_id   WHERE
                                 ...
                                 FROM
                                    tbl_purchase_detail
                                        INNER JOIN
                                  tbl_sale_return_detail  ON tbl_purchase_detail.pur_item_id = tbl_sale_return_detail.pur_item_id where tbl_sale_return_detail.sold='0' and tbl_sale_return_detail.returned='1' and tbl_sale_return_detail.iemi='0') and tbl_purchase_detail.transfer='0' Limit $qty_num";
...
$i++;
      ...
      echo '                
                        ...
                        <td hidden><input type="text"  name="qty_rec[]" id="qty_rec_'.$i.'" class="form-control calculate" required onchange="getTotal('.$i.')" value="'.$qty.'"></td>
                        <td><input type="text" readonly="" name="qty1[]" id="qty_'.$i.'" class="form-control calculate" required onkeyup="getTotal('.$i.')" value="'.$qty.'"><input type="hidden" readonly="" name="qty[]" id="qty1_'.$i.'" class="form-control calculate" required onkeyup="getTotal('.$i.')" value="'.$qty.'"></td>
                        <td>
                          <input type="text"  name="cash_rate[]" id="cash_rate_'.$i.'" required="" readonly="" class="form-control cash_rate calculate "    autocomplete="off" onchange="getTotal('.$i.')" value="'.$cash_rate.'">

                          
                        </td>
                        <td>
                        <input type="text" readonly="" name="inst_rate[]" readonly="" id="inst_rate_'.$i.'" class="form-control inst_rate"  autocomplete="off" readonly="" value="'.$inst_rate.'">
