...
if(mysqli_num_rows($result)>0){ 
     ...
     while($row = mysqli_fetch_array($result) ){
         $query_price = mysqli_query($conn,"SELECT cash_price, installment_price FROM tbl_item_price where item_id='$items' and user_id='$location'");  
        $price_data = mysqli_fetch_assoc($query_price) ;
        $cash_price=$price_data['cash_price'];
        $inst_price=$price_data['installment_price'];
        $barcode = $row['barcode'];
        $item_serial = $row['item_serial'];
        $pur_item_id = $row['pur_item_id'];
        $users_arr[] = array("cash_price" => $cash_price, "inst_price" => $inst_price, "barcode" => $barcode, "item_serial" => $item_serial, "pur_item_id" => $pur_item_id);
  ...
  $query = "SELECT barcode,item_serial,pur_item_id FROM tbl_purchase_req_detail where product = '$items' and barcode = '$barcode' and item_serial = '$item_serial' and pur_item_id = '$pur_item_id' limit 1";
