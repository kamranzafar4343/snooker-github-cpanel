...
include "../includes/session.php";
    ...
    $iemi=mysqli_real_escape_string($conn, $_POST['iemi']);
     ...
     if($iemi=='1')
    {
        $sale_type='Local Cash Sale (IEMI)';     
    }
    else
    {
        $sale_type='Local Cash Sale';    
    }
...
if($iemi=='1')
    {
        $Sale='Local Sale (IEMI)';
        $invoice_no="Local_Sale"."_".$saleid;
    }
    else
    {
     ...
     for ($a = 0; $a < count($_POST["qty"]); $a++)
        {
            ...
            $item=$item[0];
             ...
             $sql = mysqli_query($conn, "INSERT INTO tbl_sale_detail (sale_id, invoice_no, product, item_serial,pur_item_id, barcode, qty, rate, amount, created_date, created_by, parent_id, local) VALUES ('$saleid', '$invoice_no','$item','" . $_POST["item_serial"][$a] . "','" . $_POST["pur_item_id"][$a] . "','" . $_POST["barcode"][$a] . "', '" . $_POST["qty"][$a] . "',  '" . $_POST["rate"][$a] . "',  '" . $_POST["amount"][$a] . "', '$created_date', '$created_by', '$parent_id', '1')");
...
$narration="Total Amount Recieved Against ".$Sale." # ".$saleid." is ".$amount_recieved." on Date ".$invoice_date." and Customer Name is ".$customer_name." and Customer CNIC is ".$customer_cnic." (CUSTOM NARRATION= ".$remarks.")" ;
$v_type='CR';


$sql=mysqli_query($conn, "INSERT INTO tbl_trans(customer_id, invoice_no, narration, total_amount, v_type, bill_status ,payment_method ,created_date, created_by, parent_id) VALUES ('$credit_customer', '$invoice_no', '$narration', '$net_amount',  '$v_type', 'Completed', 'Cash', '$created_date', '$userid', '$parent_id')");
...
$invoice_date=mysqli_real_escape_string($conn, $_POST['invoice_date']);
            ...
            $sql = "INSERT INTO tbl_trans_detail (trans_id, invoice_no, acode, d_amount, c_amount, bill_status, narration, created_date, created_by, parent_id) VALUES ('$tranid', '$invoice_no', '$cash_sale','$net_amount', '0.00','Completed','$narration', '$created_date', '$userid', '$parent_id')";
...
else
{
 	...
 	$sql=mysqli_query($conn, "INSERT INTO tbl_sale( location,iemi,local, sale_type, sales_men, customer_name, customer_address, customer_phone, customer_cnic, customer_email, net_amount, gross_amount, discount, amount_recieved, remarks,  created_date, created_by, parent_id) VALUES ('$location', '$iemi', '1', '$sale_type', '$sales_men', '$credit_customer', '$customer_address', '$customer_phone', '$customer_cnic', '$customer_email	', '$net_amount', '$gross_amount', '$discount', '$amount_recieved', '$remarks', '$created_date', '$created_by', '$parent_id')");
