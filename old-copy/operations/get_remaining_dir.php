...
$sql=mysqli_query($conn, "SELECT product, qty FROM tbl_purchase_req_detail where purchase_req_id='$po' and pur_item_id='0'");
...
$query4=mysqli_query($conn, "SELECT sum(qty) as qty,product  FROM tbl_purchase_detail INNER JOIN tbl_items ON tbl_purchase_detail.product = tbl_items.item_id  WHERE
item_serial NOT IN (SELECT 
 ...
 AND item_serial NOT IN (SELECT 
  ...
  AND tbl_purchase_detail.qty_rec!='' and tbl_purchase_detail.product='$itemid' and tbl_purchase_detail.transfer='0'");
