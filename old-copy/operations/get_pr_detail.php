...
error_reporting(0);
     ...
     while($row = mysqli_fetch_array($result) ){
      ...
      $query1 = mysqli_query($conn,"SELECT SUM(qty) as qty FROM `tbl_purchase_req_detail` where purchase_req_id='$purchase_req_id' and pur_item_id='0'");

      $data=mysqli_fetch_assoc($query1);
      $qty1 = $data['qty'];
      
          // $query1 = mysqli_query($conn,"SELECT SUM(qty_rec) as qty FROM `tbl_purchase_req_detail` where purchase_req_id='$purchase_req_id' and qty_rec!=''");
