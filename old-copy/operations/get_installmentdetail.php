...
if($_POST['product_name'])
{
	...
	$Installment_id = $_POST['Installment_id'];
     $plan_id = $_POST['plan'];
	$query1 = "SELECT * FROM tbl_installment where item_id = '$product_name' and customer='$Installment_id' and plan_id='$plan_id'";
...
error_reporting(0);
    ...
    $sql_date=mysqli_query($conn, "SELECT to_date FROM tbl_installment_payment where plan_id = '$plan_id' order by payment_id DESC Limit 1");
    ...
    else
    {
       ...
       $users_arr[] = array("plan_id" => $plan_id, "per_month_amount" => $per_month_amount, "avo_per_amt" => $avo_per_amt, "remaining" => $remaining, "salesman_id" => $salesman_id, "salesman" => $salesman,"avo_id" => $avo_id, "avoname" => $avoname,"mo_id" => $mo_id, "moname" => $moname, "to_date" => $to_date);  
   }
