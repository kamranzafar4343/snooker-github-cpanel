...
<?php
  ...
  $("#danger-alert").hide();
    ...
    <?php }?>
                   <form action="operations/add_sale.php" method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="body">
                            <div class="row clearfix"> 
                              <div class="col-md-4 col-sm-12">
                                ...
                                <div class='col-md-8 col-sm-4' style="padding-top: 9px;padding-bottom: 4px;">
                                    ...
                                    <div class="form-group">        
                                        <select class="form-control"   required  name="sales_men" id="sales_men" name="location">
                                ...
                                <div class="col-md-4 col-sm-12">
                                     ...
                                     <div class='col-md-8 col-sm-4' style="padding-bottom: 7px;">
                                       <a href="add_clients.php?sale_type=6" >
...
if($user_privilege!='branch' && $created_by=='1')
 ...
 AND item_serial NOT IN (SELECT 
  ...
  AND tbl_purchase_detail.qty_rec!='' and tbl_purchase_detail.transfer='0' and tbl_purchase_detail.iemi='1' and tbl_purchase_detail.parent_id='$created_by'"; 
                         ...
                         $return_query="SELECT * FROM tbl_purchase_detail INNER JOIN tbl_items ON tbl_purchase_detail.product = tbl_items.item_id   WHERE
                                 ...
                                 FROM
                                    tbl_purchase_detail
                                        INNER JOIN
                                  tbl_sale_return_detail  ON tbl_purchase_detail.item_serial = tbl_sale_return_detail.item_serial where tbl_sale_return_detail.sold='0' and tbl_sale_return_detail.returned='1' and tbl_sale_return_detail.iemi='1' and tbl_sale_return_detail.parent_id='$created_by') and tbl_purchase_detail.transfer='0'";
                                }
  else
  {
...
item_serial NOT IN (SELECT 
 ...
 AND item_serial NOT IN (SELECT 
  ...
  AND tbl_purchase_req_detail.recieved!='0' and tbl_purchase_req_detail.transfer='0' and tbl_purchase_req_detail.iemi='1' and tbl_purchase_req_detail.parent_id='$created_by'"; 
                         ...
                         $return_query="SELECT * FROM tbl_purchase_req_detail INNER JOIN tbl_items ON tbl_purchase_req_detail.product = tbl_items.item_id   WHERE
                                 ...
                                 FROM
                                    tbl_purchase_req_detail
                                        INNER JOIN
                                  tbl_sale_return_detail  ON tbl_purchase_req_detail.item_serial = tbl_sale_return_detail.item_serial where tbl_sale_return_detail.sold='0' and tbl_sale_return_detail.returned='1' and tbl_sale_return_detail.iemi='1' ) and tbl_purchase_req_detail.parent_id='$created_by'";
          ...
          if($edit_id!='')
          {
                ...
                while($data=mysqli_fetch_assoc($lsql))
                {
                        ...
                        $sql5=mysqli_query($conn, "SELECT rate FROM tbl_purchase_detail where item_serial='$item_serial' and product='$product'");
...
if($row['item_id']==$product  && $row['item_serial']==$item_serial){
...
echo "<option value=$row[item_id]"." $row[barcode] "." $row[item_serial] selected>$brand_name $row[item_name] "." $row[barcode] "." $row[item_serial]</option>"; 
 ...
 ?>
                        ...
                        <td>
                          <input type="hidden" name="rate_max[]" id="rate_max_<?php echo $i;?>" class="form-control calculate" required autocomplete="off"  value="<?php echo $rate_max;?>">
...
echo "<option value=$row1[item_id]".",$row1[barcode]".",$row1[item_serial]> $brand_name $row1[item_name] $barcode1 $item_serial1</option>"; 
 ...
 ?>
                        ...
                        <td><input type="text" name="stock_qty[]" id="stock_qty_<?php echo $i;?>"  class="form-control calculate" readonly></td>
                        <td><input type="text" name="qty[]" readonly id="qty_<?php echo $i;?>"  class="form-control calculate" onchange="getTotal(<?php echo $i;?>)" ></td>
                        <td>
                        <input type="hidden" name="rate_max[]" id="rate_max_<?php echo $i;?>"  class="form-control calculate"  required  autocomplete="off" >
                          <input type="text" name="rate[]" id="rate_<?php echo $i;?>"  class="form-control calculate"  required  autocomplete="off" onchange="check_price(<?php echo $i;?>)" >
...
$(".submit").click(function() {
 ...
 if(sale_type=='Cash')
  {
    var credit_customer=$('#credit').val();
    if(credit_customer=='')
    {
...
if(Number(amount_recieved)<Number(net_amount))
{
  alert('Opps! Amount Recieved must not be less than Total amount..!');
    $('#amount_recieved').val('');
...
echo "<option value=$row1[item_id]".",$row1[barcode]".",$row1[item_serial]> $brand_name $row1[item_name] $barcode1 $item_serial1</option>"; 
 ...
 ?>'
  html += '</select>'+
                    ...
                    '<td><input type="hidden" name="rate_max[]" id="rate_max_'+row_id+'"  class="form-control" readonly   onkeyup="getTotal('+row_id+')" ><input type="text" name="rate[]" id="rate_'+row_id+'"  class="form-control"    onchange="check_price('+row_id+')" ></td>'+
 ...
 function check_price(row = null) {

    if(row) {

      var rate=$("#rate_"+row).val();
      var rate_max=$("#rate_max_"+row).val();
      if(Number(rate)<Number(rate_max))
      {
        alert("Rate must be equal or greater than purchase rate..!");
        $("#rate_"+row).val(rate_max);
        $("#rate_"+row).focus();
      }
