            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-6 col-md-8 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i class="fa fa-arrow-left"></i></a> Create New PR Transfer (IEMI)</h2>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.php"><i class="icon-home"></i></a></li>
                            <li class="breadcrumb-item">Purchase</li>
                            <li class="breadcrumb-item active">Add PR Transfer (IEMI)</li>
                        </ul>
                    </div>            
...
<?php
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                   <form action="operations/add_req_transfer.php" method="post" enctype="multipart/form-data">
                    <div class="card">
                        <div class="body">
                            <div class="row clearfix"> 
                                      ...
                                      ?>
                                        <select class="form-control po"  name="po" onchange="get_detail();" >
                                          <option selected="" value="">-- Chose One --</option>
                                           <?php
                                            $sql="SELECT * FROM tbl_purchase_req where stock_status='Pending' and iemi='1' and transfer_type='0'"; 
                                            foreach ($conn->query($sql) as $row){
...
var edit_id=$('#edit_id').val();
  ...
  $.ajax({
                url: "operations/add_req_items.php",
                type: "POST",
                data: { 
                    po : po,
                    edit_id : edit_id
...
else
{
  ...
  var dataString = 'itemid='+ itemid + '&qty='+ qty + '&po='+ po;
    ...
    $.ajax({
type: "POST",
url: "operations/get_remaining.php",
data: dataString,
...
if(responce!='')
{
    ...
    $("#stock_alert").fadeTo(4000, 500).slideUp(500, function() {
                      $("#stock_alert").slideUp(500);
                      $("#stock_alert").empty();
                    });
...
if(net_amount=='0')
{   
    alert('Opps! atleast transfer one item..!');
    return false;
}
 });
return false;
 ...
 ?>'
  ...
  function getTotal(row = null) {
    ...
    if(row) {
      ...
      var pur_rate=$("#pur_rate_"+row).val();
      var rate=$("#rate_"+row).val();
      if(rate=='' || isNaN(rate) || rate=='0')
      {
        $("#rate_"+row).val(pur_rate);
      }
       if(Number(rate) < Number(pur_rate))
      {
        alert('Sorry !! Please Enter Rate Equal or Greater than Purchase Rate..');
        $("#rate_"+row).val(pur_rate);
