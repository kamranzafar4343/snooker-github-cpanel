                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr data-status="assets" style="display: none">
                                                ...
                                                <td><span class="badge badge-info">Opening <h6><?php if ($opening_bal) {
                                                                                                    echo $opening_bal;
                                                                                                } else {
                                                                                                    echo 0;
                                                                                                } ?></span></h6>
                                        ...
                                        ?>
                                            <tr data-status="assets" style="display: none">
                                                ...
                                                <td><?php echo $aname2;
                                                    if ($aname1 != '') { ?> ( <?php echo $aname1; ?> ) <?php } ?></td>
                                        ...
                                        <?php
                                        $count = 0;
                                        $sql = mysqli_query($conn, "SELECT * FROM tbl_account where LEFT(acode,3)='200' order by id asc");
                                        while ($pdata = mysqli_fetch_assoc($sql)) {
                                            $acode = $pdata['acode'];
                                            $aname = $pdata['aname'];
                                            $atype = $pdata['atype'];
                                        ...
                                        <?php
                                        $count = 0;
                                        $sql = mysqli_query($conn, "SELECT * FROM tbl_account where LEFT(acode,3)='400' and LEFT(acode, 6)!= '400300' order by id asc");
                                        while ($pdata = mysqli_fetch_assoc($sql)) {
                                            $acode = $pdata['acode'];
                                            $aname = $pdata['aname'];
                                            $id = $pdata['id'];
                                        ...
                                        <?php
                                        $count = 0;
                                        $sql = mysqli_query($conn, "SELECT * FROM tbl_account 
WHERE LEFT(acode, 3) = '300' 
   OR LEFT(acode, 6) = '400300' 
ORDER BY id ASC;");
                                        while ($pdata = mysqli_fetch_assoc($sql)) {
                                            $acode = $pdata['acode'];
                                            $aname = $pdata['aname'];
                                            ...
                                            // Check for Sales account (excluding returns)
                                            if (strtolower($aname) === 'sales' || ($acode == '300100' && stripos($aname, 'return') === false)) {
                                                $showTotal = true;
                                                $totalLabel = 'Sales';  // Custom label
                                                $badgeClass = 'badge badge-success';
                                                $sql_amount = mysqli_query($conn, "SELECT SUM(gross_amount) as total FROM tbl_sale 
            WHERE parent_id='$created_by'");
                                            }
                                            // Check specifically for Sales Returns account
                                            elseif (strtolower($aname) === 'sales returns') {
                                                $showTotal = true;
                                                $totalLabel = 'Sales Returns';  // Custom label
                                                $badgeClass = 'badge badge-danger';
                                                $sql_amount = mysqli_query($conn, "SELECT SUM(amount_returned) as total FROM tbl_sale_return 
            WHERE parent_id='$created_by'");
                                            }
                                            // Check for Cost of Goods Sold account
                                            elseif (strtolower($aname) === 'cost of goods sold') {
                                                $showTotal = true;
                                                $totalLabel = 'Cost of Goods Sold';  // Custom label
                                                $badgeClass = 'badge badge-warning';
                                                $sql_amount = mysqli_query($conn, "SELECT SUM(tbl_items.purchase) AS total
            FROM tbl_sale_detail
            JOIN tbl_items ON tbl_sale_detail.product = tbl_items.item_id;");
                                            }

                                            // Fetch the total amount
                                            if ($showTotal && isset($sql_amount)) {
                                                $amount_data = mysqli_fetch_assoc($sql_amount);
                                                $totalAmount = $amount_data['total'] ?? 0;
                                            }
                                        ...
                                        ?>
                                            ...
                                            <tr data-status="revenue" style="display: none">
                                                <td><?php echo htmlspecialchars($customername1 ?? ''); ?></td>
                                                <td><?php echo htmlspecialchars($aname); ?></td>

                                                <td>
                                                    <ul class="list-unstyled team-info">
                                                        <li><img src="<?php echo htmlspecialchars($user_profile); ?>"
                                                                data-toggle="tooltip"
                                                                data-placement="top"
                                                                title="Created by <?php echo htmlspecialchars($user_name); ?>"
                                                                alt="Created by <?php echo htmlspecialchars($user_name); ?>"></li>
                                                    </ul>
                                                </td>

                                                <td>
                                                    <span class="badge badge-info">Opening
                                                        <h6><?php echo $opening_bal ? number_format($opening_bal, 2) : '0.00'; ?></h6>
                                                    </span>
                                                </td>

                                                <td>
                                                    <?php if ($showTotal && $totalAmount != 0): ?>
                                                        <span class="badge <?php echo $badgeClass; ?> big-badge">
                                                            <?php echo $totalLabel; ?>: <?php echo number_format(abs($totalAmount), 2); ?>
                                                        </span>
                                                    <?php endif; ?>
                                                </td>
                                                ...
                                                <?php if ($c_write == '1') { ?>
                                                    <td class="project-actions">
                                                        <a href="add_account.php?subhead_id=<?php echo $id; ?>"
                                                            class="btn btn-sm btn-outline-success">
                                                            <i class="icon-pencil"></i>
                                                        </a>
                                                        <a href="operations/delete.php?subhead_id=<?php echo $id; ?>"
